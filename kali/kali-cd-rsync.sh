...
RSYNCSOURCE=#<rsync URL, for example: mirrors.ocf.berkeley.edu::kali-images>
BASEDIR=#<directory for rsync to download to, for example /srv/mirror/kali-images>
...
FILENAME=kali-cd-rsync-$DAY.log
LOGPATH=#<Path to log directory>/kali-cd/$FILENAME
...
if [ -d ${BASEDIR} ] ; then
	...
	touch ${LOCK}
	echo "Running Kali CD sync" >> "$LOGPATH"
