...
RSYNCSOURCE=#<rsync URL, for example: cdimage.debian.org::debian-cd>
BASEDIR=#<directory for rsync to download to, for example /srv/mirror/debian-cd>
...
FILENAME=debian-cd-rsync-$DAY.log
LOGPATH=#<Path to log directory>/debian-cd/$FILENAME
...
if [ -d ${BASEDIR} ] ; then
	...
	echo "Target directory /var/www/html/debian-cd not present."
fi
